...
from .errors import (
    ERR_404_EXPORT_NOT_FOUND, ERR_404_NO_ACTIVE_CLOSURE,
    ERR_409_CLOSURE_PENDING, ERR_409_EXPORT_COOLDOWN,
    ERR_410_DOWNLOAD_EXPIRED, ERR_425_EXPORT_NOT_READY,
)
from .models import AccountClosureRequest, DataExportRequest
from .orchestrator import gdpr_orchestrator
from .serializers import ClosureStatusSerializer, ExportRequestSerializer, ExportStatusSerializer
    ...
    def post(self, request: Request):
        try:
            ...
            if str(e) == 'export_cooldown':
                return IronErrorResponse(409, ERR_409_EXPORT_COOLDOWN)
        ...
        return IronResponse(ExportRequestSerializer(dto), status=202)
...
class DataExportStatusView(APIView):
    ...
    def get(self, request: Request):
        ...
        ).exclude(status=DataExportRequest.STATUS_EXPIRED).order_by('-created_at').first()

        if not export_req:
            return IronErrorResponse(404, ERR_404_EXPORT_NOT_FOUND)
        ...
        expires_at  = (
            ...
            if export_req.download_expires_at else None
        )
        ...
        return IronResponse(ExportStatusSerializer(dto))
...
class DataExportDownloadView(APIView):
    ...
    def get(self, request: Request):
        ...
        if export_req.status != DataExportRequest.STATUS_READY:
            return IronErrorResponse(425, ERR_425_EXPORT_NOT_READY)

        if export_req.download_expires_at and timezone.now() > export_req.download_expires_at:
            export_req.status = DataExportRequest.STATUS_EXPIRED
            export_req.save(update_fields=['status'])
            return IronErrorResponse(410, ERR_410_DOWNLOAD_EXPIRED)

        if not export_req.archive_path or not os.path.exists(export_req.archive_path):
            logger.error('GDPR archive file missing: request=%s path=%s', export_req.pk, export_req.archive_path)
            return error_500_internal()
    ...
    def post(self, request: Request):
        try:
            ...
            if str(e) == 'closure_already_pending':
                return IronErrorResponse(409, ERR_409_CLOSURE_PENDING)
            return error_500_internal()
        ...
        return IronResponse(ClosureStatusSerializer(dto), status=202)
...
class AccountCancelCloseView(APIView):
    ...
    def post(self, request: Request):
        try:
            ...
            return IronErrorResponse(404, ERR_404_NO_ACTIVE_CLOSURE)
        ...
        return IronResponse(ClosureStatusSerializer(dto))
...
class AccountCloseStatusView(APIView):
    ...
    def get(self, request: Request):
        closure = AccountClosureRequest.objects.filter(
            user_id=request.user.pk,
        ).exclude(status=AccountClosureRequest.STATUS_CANCELLED).order_by('-initiated_at').first()
...
class ExportPartReadyView(APIView):
    ...
    permission_classes = [permissions.IsAuthenticated]  # replaced by IsServiceRequest in production
    ...
    def post(self, request: Request, request_id: int):
        ...
        if not service:
            return IronErrorResponse(400, 'error.400.bad_request')

        try:
            gdpr_orchestrator.mark_part_ready(request_id, service)
        except Exception as e:
            logger.error('mark_part_ready failed: request=%s service=%s err=%s', request_id, service, e)
            return error_500_internal()
