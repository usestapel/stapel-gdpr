...
| Boot-time configuration checks | `checks.py` (`gdpr.E001/E002/W003/E004/W005/E006/W006/W007/W008/E009/E010/W011/W012`), registered from `apps.ready()` | `manage.py check` fails on a missing/stale data-owner inventory, an owner name no installed library declares (`E009`), an installed owner the inventory omits (`E010`) or unverified hash rows, and warns on every open escape hatch, every deliberate owner opt-out (`W011`), every subject an owner erases and the host never asks about (`W012`), every owner that stopped answering probes (`W006`) and every unacknowledged data-subject request (`W008`) |
