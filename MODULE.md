...
| Data-owner name agreement | `declarations.installed_owner_declarations()`, `gdpr.E009/E010/W011/W012`, `STAPEL_GDPR["DATA_OWNERS_OPT_OUT"]` | The host's inventory is compared with what the **installed libraries** declare (`register_gdpr_owner`, then `GDPRProvider.section`, then the module constants `OWNER`/`SUBJECT_TYPES`). An app label is not an owner name — the `cdn` app owns `media` — and a library the inventory omits is a store no erasure ever waits for, which has no symptom at all |
...
| Boot-time configuration checks | `checks.py` (`gdpr.E001/E002/W003/E004/W005/E006/W006/W007/W008/E009/E010/W011/W012/E013/W013/W014`), registered from `apps.ready()` | `manage.py check` fails on a missing/stale data-owner inventory, an owner name no installed library declares (`E009`), an installed owner the inventory omits (`E010`), unverified hash rows, or an export root inside a served root (`E013` — a personal-data archive at an unauthenticated, cacheable URL), and warns on every open escape hatch, every deliberate owner opt-out (`W011`), every subject an owner erases and the host never asks about (`W012`), every owner that stopped answering probes (`W006`), every unacknowledged data-subject request (`W008`), an unnamed export root (`W013` — archives local to whichever process wrote them) and peer export slices landing in a served `default_storage` (`W014`) |
...
|---|---|---|
| `DATA_OWNERS` | `[]` | **Mandatory inventory.** A map owner → subject types: `{"recordings": ["account", "workspace", "recording"]}`; a value may be a full spec (`{"subject_types", "kind": "local"\|"remote", "timeout_hours"}`). A plain list is still accepted and means `["account"]` for every entry. One `ErasurePart` per owner **claiming the subject**; an empty inventory is a `gdpr.E001` boot error and blocks every `deleted` state. Names must be the ones the libraries declare, not app labels (`gdpr.E009`), and every installed owner must be listed (`gdpr.E010`) |
| `DATA_OWNERS_OPT_OUT` | `[]` | Owner names an **installed** library declares that this deployment deliberately does not ask. Downgrades `gdpr.E010` to `gdpr.W011` for those names — a chosen exemption stays reported at every boot rather than becoming silence |
...
| `EXPORT_ROOT` | `""` (→ `BASE_DIR/private/gdpr`) | **Private root for personal-data exports — never a served root.** A value inside `MEDIA_ROOT`/`STATIC_ROOT`/`STATICFILES_DIRS` is refused at boot (`gdpr.E013`); unset is `gdpr.W013`, since the default is local to whichever process wrote it. See CONFIG.MD, "Where an export lives" |
| `STAGING_ROOT` | `""` (→ `EXPORT_ROOT/staging`) | Per-directory override for the staging dirs; `gdpr.E013` checks it too |
| `ARCHIVE_ROOT` | `""` (→ `EXPORT_ROOT/exports`) | Per-directory override for the finished ZIPs; `gdpr.E013` checks it too |
...
| `GDPR_EXPORT_ROOT` / `GDPR_STAGING_ROOT` / `GDPR_ARCHIVE_ROOT` | `export_store.py` | Flat equivalents of `EXPORT_ROOT` / `STAGING_ROOT` / `ARCHIVE_ROOT` |
| `STORAGES["stapel_gdpr_exports"]` | `export_store.export_storage()` | The export store when the celery worker and the web process do not share a filesystem. Unset, the module uses a private `FileSystemStorage` rooted at `EXPORT_ROOT` whose `url()` raises |
